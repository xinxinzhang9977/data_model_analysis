            ...
            for atom in self.center.atoms:
                if atom.resid not in self.exist:
                    ...
                    while self.head < self.tail:
                        ...
                        self.head += 1
                        if self.find_name(center_now.resids[0])=='MG' or self.find_name(center_now.resids[0])=='TFSI':
                            if self.find_name(center_now.resids[0])=='MG':  # 是Mg,扩展寻找，但Mg周围的所有东西都入队
                                selection = f'around 2.8 group center_now'  # 所有东西都入队
                            elif self.find_name(center_now.resids[0])=='TFSI':
                    ...
                    #print(' '.join(a))
                    #print('\n')
                    valence = 2*cluster_data['MG'] - cluster_data['TFSI']
                    # 输出中性团簇的坐标文件pdb
                    if valence == 99:
                        out_counter += 1
...
if __name__ == "__main__":
    ...
    for filepath in filepaths:
        cluster = Cluster_stat(filepath, 'resid 801 to 860')
        cluster.main()
    end = time.time()
