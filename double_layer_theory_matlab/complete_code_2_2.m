...
v  = 4/3*R^3*pi;            % ion volume
c0 = 5000;               % bulk concentration (SAFE < close packing)
qs = 0.1;                 % surface charge density (C/m^2)
...
for it = 1:maxIter
    ...
    c_plus  = c0 * exp(-beta*e*phi_bar - beta*mu_ex_bar+beta*mu_ex);
    c_minus = c0 * exp(+beta*e*phi_bar - beta*mu_ex_bar+beta*mu_ex);
