...
c0 = 5000*NA;               % bulk concentration (SAFE < close packing)
qs = 0.6;                 % surface charge density (C/m^2)
p0 = v * (2*c0);
mu_ex_bulk = (8*p0 - 9*p0^2 + 3*p0^3) / (1-p0)^3;
...
for it = 1:maxIter
    ...
    c_plus  = c0 * exp(-beta*e*phi_bar - mu_ex_bar + mu_ex_bulk);
    c_minus = c0 * exp(+beta*e*phi_bar - mu_ex_bar + mu_ex_bulk);
    ...
    p_bar = min(p_bar, 0.95);
    mu_ex_bar_old = mu_ex_bar;
    mu_ex = (8*p_bar - 9*p_bar.^2 + 3*p_bar.^3) ./ (1 - p_bar).^3;
    mu_ex_bar_new = Wv * mu_ex;

    mu_ex_bar = (1-omega)*mu_ex_bar_old + omega*mu_ex_bar_new;
...
%% =========================================================
phi_bar = Ws * phi;
c_plus  = c0 * exp(-beta*e*phi_bar-mu_ex_bar);
c_minus = c0 * exp(+beta*e*phi_bar-mu_ex_bar);
...
figure;
plot(x*1e9, c_plus/c0, 'LineWidth', 2)
hold on;
plot(x*1e9, c_minus/c0, 'LineWidth', 2)
xlabel('x (nm)')
...
% Build nonlocal weight matrices following
% de Souza et al., PRL 125, 146001 (2020), SM Numerical Implementation
%
% 1D slab geometry, hard-sphere radius R
...
for i = 1:Nx
    for j = 1:Nx
        ...
        if dz <= R
            % ---- surface (charge) weight w_s ----
            Ws(i,j) = 1/(2*R);

            % ---- volume (packing) weight w_v ----
            Wv(i,j) = (3/(4*R^3)) * (R^2 - dz^2);
        end
    end
end

% ---- discretized integral (quadrature) ----
Ws = Ws * dx;
Wv = Wv * dx;

% ---- renormalization near boundaries (SM requirement) ----
for i = 1:Nx
    s = sum(Ws(i,:));
    if s > 0
        Ws(i,:) = Ws(i,:) / s;
