...
for it = 1:maxIter
    ...
    p_bar_c = v_c * Wv_c * c_plus;
    p_bar_c = min(p_bar_c, 0.95);   % avoid CS divergence
    p_bar_a = v_a * Wv_a *c_minus;
    p_bar_c = min(p_bar_c, 0.95);   % avoid CS divergence
    % ---- Carnahan–Starling excess chemical potential ----
    mu_ex_c = (8*p_bar_c - 9*p_bar_c.^2 + 3*p_bar_c.^3) ./ (1 - p_bar_c).^3/beta;
    mu_ex_a = (8*p_bar_a - 9*p_bar_a.^2 + 3*p_bar_a.^3) ./ (1 - p_bar_a).^3/beta;
    % ---- nonlocal excess chemical potential ----
    mu_ex_bar_c = Wv_c * mu_ex_c;
    mu_ex_bar_a = Wv_a * mu_ex_a;
