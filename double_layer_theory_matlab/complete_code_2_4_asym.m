...
for it = 1:maxIter
    ...
    arg_minus = +beta*e*phi_bar_a - beta*mu_ex_bar_c;
...
c_minus(~mask_a) = 0;
    ...
    rho_bar = e * (Ws_c*c_plus - Ws_a*c_minus);
    rho_eff = rho_bar;
...
figure;
plot(x, c_plus/c0_c, 'LineWidth', 2)
hold on;
plot(x, c_minus/c0_c, 'LineWidth', 2)
ylim([0,20]);
xlabel('x (nm)')
...
function [Ws, Wv] = build_weights(x, R)
    ...
    for i = 1:Nx
        for j = 1:Nx
            ...
            if d <= R
                Wv(i,j) = pi*(R^2-d^2)/(4/3*R^3);%Wv(i,j) = 3*(R^2-d^2)/(4*R^3);
            end
        end
    end
